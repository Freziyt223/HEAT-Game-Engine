...
pub fn build(b: *std.Build) void {
    ...
    const Options = .{
        ...
        .enable_zigzag = b.option(bool, "enable_zigzag", "Specify if engine should come with zigzag TUI") orelse config.Enable.zigzag,
        .enable_vulkan = b.option(bool, "enable_vulkan", "Specify if engine should come with vulkan renderer") orelse config.Enable.vulkan,
        .singlethreaded = b.option(bool, "singlethreaded", "Specify if engine should come as singlethreaded or multithreaded") orelse config.singlethreaded,
    };
    ...
    const zglfw = b.dependency("zglfw", .{
        ...
        .shared = false, // if program doesn't even startup and you get 0xc000135 leave this as false
    });
    ...
    b.installArtifact(addExecutable(b, .{
        .name = "HEAT",
...
pub fn addExecutable(engine_builder: *std.Build, options: ExecutableOptions) *std.Build.Step.Compile {
    ...
    const IO = engine_builder.modules.get("IO").?;
    ...
    options.user_module.addImport("Engine", Root);
